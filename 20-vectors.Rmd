...
- To filter vectors, we use the subsetting function -> `[` and is called like `x[a]`.There are four types of things that we can subset a vector with:
...
3. If we have a named vector, we can subset it with a character vector:
...
```{r ex20.4.3_set_names_fn3, eval=FALSE}
...
```{r ex20.4.3_set_names_fn4, eval=FALSE}
...
```{r ex20.4.3_set_names_fn5, eval=FALSE}
...
```{r ex20.4.3_setNames_fn2, eval=FALSE}
setNames(1:4, c("a", "b"))
```
...
- Lists can contain other lists implying that they are more complex than atomic vectors.
...
- We use the function `str()` to assess the structure of lists.
...
3. `$` is a shorthand to extract named elements of a list. It works similarly to `[[` except that we don’t need to use quotes.
